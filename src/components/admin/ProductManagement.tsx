...
const ProductManagement = () => {
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">إدارة الهاكات</h2>
          <p className="text-gray-300 mt-1">إضافة وتعديل وحذف المنتجات</p>
        </div>
        <Button onClick={handleAddProduct} className="bg-gaming-gradient hover:shadow-lg hover:shadow-purple-500/25" disabled={loading}>
        ...
        <CardContent className="pt-6 bg-slate-950">
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              ...
              <Input id="search" placeholder="ابحث باسم المنتج أو الوصف..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="mt-1 bg-gray-800/50 border-gray-600 text-white" disabled={loading} />
            </div>
          </div>
        </CardContent>
      </Card>
                  ...
                  {filteredProducts.map(product => <TableRow key={product.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          <Button variant="outline" size="sm" onClick={() => handleEditProduct(product)} className="border-blue-500 text-green-200 px-[13px] py-[16px] my-[10px] mx-[12px] bg-[#4bf41f]/10" disabled={actionLoading === product.id}>
                            <Edit className="w-4 h-4" />
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => handleDeleteProduct(product.id)} className="border-red-500 font-normal text-red-600 bg-[#e11b1b]/10" disabled={actionLoading === product.id}>
