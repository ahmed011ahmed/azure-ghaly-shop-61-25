...
import { Send, MessageSquare, Trash2, User, Loader2, Image, Gamepad2 } from 'lucide-react';
import { Button } from '../ui/button';
import { Input } from '../ui/input';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../ui/card';
import { ScrollArea } from '../ui/scroll-area';
import { usePubgChat } from '../../contexts/PubgChatContext';

const PubgAdminChat = () => {
  const { messages, addMessage, clearMessages, loading } = usePubgChat();
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        const messageText = `[صورة PUBG]: ${imageData}`;
  ...
  const isImageMessage = (text: string) => {
    return text.startsWith('[صورة PUBG]: ');
  };
  ...
  const renderMessageContent = (text: string) => {
    if (isImageMessage(text)) {
      const imageData = text.replace('[صورة PUBG]: ', '');
      return (
        <div className="mt-2">
          <img 
            src={imageData} 
      ...
      <CardHeader className="bg-slate-900 flex-shrink-0 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <CardTitle className="text-white text-sm">شات إدارة PUBG</CardTitle>
              <CardDescription className="text-gray-300 text-xs">
                التواصل مع عملاء PUBG ({messages.length} رسالة)
              </CardDescription>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-300 text-xs">لا توجد رسائل PUBG بعد</p>
              </div>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center space-x-1 mb-1 flex-wrap">
                      <span className={`text-xs font-medium ${
                        ...
                        {message.sender === 'admin' ? 'دعم PUBG' : message.userName || 'مستخدم'}
                      </span>
        ...
        <div className="border-t border-gray-700 p-2 flex-shrink-0">
          ...
          <form onSubmit={handleSendMessage} className="flex space-x-1">
            <Input
              ...
              placeholder={`رسالة ${isUserMode ? 'كمستخدم' : selectedCustomer ? `لـ ${selectedCustomer}` : 'عامة للعملاء'}...`}
