...
import { Plus, Edit, Trash2, Download, Loader2, Star } from 'lucide-react';
...
interface LevelDownloadsManagementProps {
  level: number;
}

const LevelDownloadsManagement = ({ level }: LevelDownloadsManagementProps) => {
  const { downloads, loading, addDownload, updateDownload, deleteDownload } = useDownloads(level);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.description || !formData.download_url || !formData.version) return;

    try {
      const downloadData = { ...formData, target_level: level };
      
      if (editingDownload) {
        await updateDownload(editingDownload.id!, downloadData);
      } else {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-white">
              {editingDownload ? 'تعديل تحميل' : 'إضافة تحميل جديد'} - {getLevelName(level)}
            </h2>
          </div>
          <Button
            onClick={() => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Star className={`w-6 h-6 ${getLevelColor(level)}`} />
          <h2 className="text-2xl font-bold text-white">
            تحميلات {getLevelName(level)} ({downloads.length})
          </h2>
        </div>
        <Button
      ...
      <Card className="gaming-card">
        <CardHeader className="bg-slate-900">
          <CardTitle className="text-white">روابط التحميل المخصصة للمستوى {getLevelName(level)}</CardTitle>
          <CardDescription className="text-gray-300">
            جميع روابط التحميل المتاحة لمشتركي المستوى {getLevelName(level)}
          </CardDescription>
        </CardHeader>
        <CardContent className="bg-slate-950">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-gray-300 mr-3">جاري تحميل التحميلات...</span>
            </div>
          ) : (
          ...
          {!loading && downloads.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-300 mb-2">لا توجد تحميلات</h3>
              <p className="text-gray-500 mb-4">لم يتم إضافة أي تحميلات لهذا المستوى بعد</p>
...
export default LevelDownloadsManagement;
